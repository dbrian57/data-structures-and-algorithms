...
const toLastNames = people => {
  let combinedNames = people.map(person => person.firstName + ' ' + person.lastName);
...
const addPurchases = (arr) => {
  const initialValue = 0;
   let newArray = arr.reduce((lastPrice, nextPrice) => 
    { return lastPrice + nextPrice.purchasePrice}, initialValue);
    return newArray;
  };
...
const countNumberOfElements = (arr) => {
  let numberElements = arr.reduce((previousElement, element) => {
    element = 1;
...
const returnNames = (arr) => {
  let names = arr.reduce((newCharacter, character) => {
    newCharacter.push(character.name);
    return newCharacter
  }, []);
  return names;
};
...
const reversedString = (str) => {
  let newStr = str.reduce((prevLetter, letter) => {
